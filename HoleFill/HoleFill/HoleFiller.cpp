...
void HoleFiller::refine(int idx1 , int idx2 ,int idx3)
{
    if (!mesh->triangleExists(idx1, idx2, idx3)){
        cout<<"not exist"<<std::endl; return;}
    ...
    float *coord_center= new float[3];
    coord_center[0]=(coord1[0] + coord2[0] + coord3[0]) / 3;
    coord_center[1]=(coord1[1] + coord2[1] + coord3[1]) / 3;
    coord_center[2]=(coord1[2] + coord2[2] + coord3[2]) / 3;
    ...
    float temp1 = scale * mesh->distanceBetween(coord_center, v1->coords);
    float temp2 = scale * mesh->distanceBetween(coord_center, v2->coords);
    float temp3 = scale * mesh->distanceBetween(coord_center, v3->coords);
       ...
       (temp3 > sigma_vertices[2] && temp3 > sigma_centroid) )
    {
        ...
        int idx_center = mesh->addVertex(coord_center);

        mesh->removeTriangle(idx1, idx2, idx3);
        
        mesh->addTriangle(idx1, idx_center, idx3);
        mesh->addTriangle(idx2, idx_center, idx3);
        mesh->addTriangle(idx1, idx_center, idx2);
        mesh->d_holes.push_back(new Triangle(idx1,idx_center,idx3));
        mesh->d_holes.push_back(new Triangle(idx2,idx_center,idx3));
        mesh->d_holes.push_back(new Triangle(idx1,idx_center,idx2));

    }
}

void HoleFiller::mesh_smooth(){

    
    float sumx,sumy,sumz;
  for(int h=0;h<v_holes.size();h++){
        for(int j=0;j<v_holes[h]->size();j++){
            
        sumx=0;sumy=0;sumz=0;
        for(int b=0;b<mesh->verts[(*v_holes[h])[j]->idx]->vertList.size();b++){
            float r=mesh->distanceBetween(mesh->verts[(*v_holes[h])[j]->idx]->coords,mesh->verts[mesh->verts[(*v_holes[h])[j]->idx]->vertList[b]]->coords );
        sumx+=mesh->verts[mesh->verts[(*v_holes[h])[j]->idx]->vertList[b]]->coords[0];
        sumy+=mesh->verts[mesh->verts[(*v_holes[h])[j]->idx]->vertList[b]]->coords[1];
        sumz+=mesh->verts[mesh->verts[(*v_holes[h])[j]->idx]->vertList[b]]->coords[2];

        }
        
        mesh->verts[(*v_holes[h])[j]->idx]->coords[0]=sumx/mesh->verts[(*v_holes[h])[j]->idx]->vertList.size();
        mesh->verts[(*v_holes[h])[j]->idx]->coords[1]=sumy/mesh->verts[(*v_holes[h])[j]->idx]->vertList.size();
        mesh->verts[(*v_holes[h])[j]->idx]->coords[2]=sumz/mesh->verts[(*v_holes[h])[j]->idx]->vertList.size();

    }
    }
   
    for(int a=2205;a<2239;a++){
        sumx=0;sumy=0;sumz=0;
        cout<<"index: "<<a;
        for(int b=0;b<mesh->verts[a]->vertList.size();b++){
            cout<<" "<<mesh->verts[a]->vertList[b];
            float r=mesh->distanceBetween(mesh->verts[a]->coords,mesh->verts[mesh->verts[a]->vertList[b]]->coords );
            sumx+=mesh->verts[mesh->verts[a]->vertList[b]]->coords[0];
            sumy+=mesh->verts[mesh->verts[a]->vertList[b]]->coords[1];
            sumz+=mesh->verts[mesh->verts[a]->vertList[b]]->coords[2];
            
        }
        cout<<std::endl;
        mesh->verts[a]->coords[0]=sumx/mesh->verts[a]->vertList.size();
        mesh->verts[a]->coords[1]=sumy/mesh->verts[a]->vertList.size();
        mesh->verts[a]->coords[2]=sumz/mesh->verts[a]->vertList.size();
